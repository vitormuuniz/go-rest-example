...
type CategoriesRepository interface {
	...
	FindById(uint64) (*models.Category, error)
...
func (r *categoriesRepositoryImpl) FindById(id uint64) (*models.Category, error) {
	category := &models.Category{}

	err := r.db.Debug().Model(&models.Category{}).Where("id = ?", id).Take(&category).Error
	if err != nil {
		return nil, err
	}
